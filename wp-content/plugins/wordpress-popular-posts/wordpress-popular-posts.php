	...
	class WordpressPopularPosts extends WP_Widget {
		...
		private function __render_popular_post($p, $instance) {
			...
			if ($instance['markup']['custom_html']) {
				...
				$data = array(
					'title' => '<a href="'.$permalink.'" title="'. esc_attr($title) .'" class="wpp-post-title" target="' . $this->user_settings['tools']['link']['target'] . '">'.$title_sub,
					'summary' => $excerpt,
					'stats' => $_stats,
					'img' => ( !empty($thumb) ) ? '<a class="thumb-img-side" href="'.$permalink.'" title="'. esc_attr($title) .'" target="' . $this->user_settings['tools']['link']['target'] . '">' . $thumb . '</a>' : '',
					'img_no_link' => $thumb,
					'id' => $p->id,
					'url' => $permalink,
		...
		protected function _get_excerpt($p, $instance) {
			...
			if ($instance['post-excerpt']['active']) {
				...
				if (!empty($excerpt) && !$instance['markup']['custom_html']) {
					$excerpt = '<span class="wpp-excerpt">' . $excerpt . '</span></a>';
				}
